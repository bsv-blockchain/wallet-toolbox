import { TableTransaction, WalletStorageManager } from '../../..'
import {
  _tu,
  TestWalletNoSetup
...
dotenv.config()
describe('processAction.test.ts', () => {
  ...
  test('TODOTONE Throw error if rawTx is provided but is not a valid signed transaction with legacy DB', async () => {
    for (const { identityKey, wallet, activeStorage: storage } of ctxs) {
      ...
      const rawTxHex = `0100000001${fundingTxidLE}${fundingVoutLE}00FFFFFFFF01E7030000000000001976A9146F7E1838868BCD8E4BE20B5E0364CA479C88D92A88AC00000000`
      ...
      /* This transaction could be invalid due to an error in the script, TBD

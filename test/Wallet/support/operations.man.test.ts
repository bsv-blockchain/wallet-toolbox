...
import { sdk, Services, Setup, StorageKnex, TableOutput, TableTransaction, TableUser, verifyOne, verifyOneOrNone } from '../../../src'
...
describe('operations.man tests', () => {
  ...
  test('9 review recent transaction change use', async () => {
    const { env, storage, services } = await createMainReviewSetup()
    const countTxs = await storage.countTransactions({ partial: { userId: 311 }, status: ['completed', 'unproven', 'failed'] })
    const txs = await storage.findTransactions({ partial: { userId: 311 }, status: ['unproven', 'completed', 'failed'], paged: { limit: 100, offset: Math.max(0, countTxs - 100) } })
    for (const tx of txs) {
      const ls = await toLogString(tx, storage)
      console.log(ls)
    }
    const countReqs = await storage.countProvenTxReqs({ partial: {}, status: ['completed', 'unmined'] })
    const reqs = await storage.findProvenTxReqs({ partial: {}, status: ['unmined', 'completed'], paged: { limit: 100, offset: countReqs - 100 } })
    await storage.destroy()
  })
})
